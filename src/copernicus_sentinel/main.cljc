(ns copernicus_sentinel.main
  "Kotodama WASM entrypoint for the Copernicus_sentinel clean-room actor (L4) — Clojure port.
  ...
  facts (`copernicus_sentinel.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "copernicus_sentinel")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"copernicus_sentinel.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "copernicus_sentinel-compat" :tier tier :entities entities} 200])
...
(defn start! [] :copernicus_sentinel-compat/ready)
