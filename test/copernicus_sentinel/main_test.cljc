(ns copernicus_sentinel.main-test
  "Contract + behavioral test for the copernicus_sentinel-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "copernicus_sentinel-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'copernicus_sentinel.main-test)]
